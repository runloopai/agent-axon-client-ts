...
function summarizeAxonEvent(event: AxonEventView): AxonEventSummary {
  ...
  switch (event.event_type) {
    ...
    case "session/new": {
      ...
      const modes = parsed.modes as { availableModes?: Array<{ id: string }> } | undefined;
      const modeList = modes?.availableModes?.map((m) => m.id).join(", ") ?? "";
      const detail = cwd ? `cwd: ${cwd}` : sid ? `session: ${sid.slice(0, 16)}` : "";
      const modeSuffix = modeList ? ` [${modeList}]` : "";
      return { icon: "\u{1F195}", label: "New Session", summary: `${detail}${modeSuffix}`, colorClass: baseColor };
    }
    case "session/prompt": {
      if (origin.includes("user")) {
        const prompt = parsed.prompt as Array<{ type: string; text?: string }> | undefined;
        const text = prompt?.find((p) => p.type === "text")?.text ?? "";
        const preview = text.length > 80 ? text.slice(0, 80) + "\u2026" : text;
        return { icon: "\u{1F4AC}", label: "Prompt", summary: preview, colorClass: "origin-user" };
      }
      const stopReason = (parsed.stopReason as string) ?? "complete";
      return { icon: "\u2705", label: "Turn Complete", summary: stopReason, colorClass: "origin-agent" };
    }
    case "turn.started":
      return { icon: "\u25B6\uFE0F", label: "Turn Started", summary: `turn_id: ${parsed.turn_id ?? ""}`, colorClass: "origin-system" };
    case "turn.completed": {
      const reason = (parsed.stop_reason as string) ?? "";
      return { icon: "\u23F9\uFE0F", label: "Turn Completed", summary: reason, colorClass: "origin-system" };
    }
    case "session/update": {
      ...
      switch (update.sessionUpdate) {
        case "agent_message_chunk": {
          ...
          return { icon: "\u{1F4DD}", label: "Agent Message", summary: preview, colorClass: "origin-agent" };
        }
        case "agent_thought_chunk": {
          const text = update.content.type === "text" ? update.content.text : "";
          const preview = text.length > 60 ? text.slice(0, 60) + "\u2026" : text;
          return { icon: "\u{1F4AD}", label: "Thinking", summary: preview, colorClass: "origin-thinking" };
        }
        case "tool_call": {
          const kind = update.kind ?? "";
          return { icon: "\u{1F527}", label: "Tool Call", summary: `${update.title}${kind ? ` (${kind})` : ""}`, colorClass: "origin-agent" };
        }
        case "tool_call_update": {
          ...
          return { icon: failed ? "\u274C" : "\u{1F527}", label: "Tool Update", summary: `${detail} \u2014 ${status}`, colorClass: failed ? "origin-error" : "origin-agent" };
        }
        case "current_mode_update":
          return { icon: "\u{1F500}", label: "Mode", summary: update.currentModeId, colorClass: "origin-system" };
        case "plan":
          return { icon: "\u{1F4CB}", label: "Plan", summary: `${update.entries.length} steps`, colorClass: "origin-agent" };
        case "usage_update":
          return { icon: "\u{1F4CA}", label: "Usage", summary: `${update.used}/${update.size} tokens`, colorClass: "origin-system" };
        case "available_commands_update": {
          const cmds = (update as unknown as { availableCommands: Array<{ name: string }> }).availableCommands ?? [];
          return { icon: "\u{1F4CB}", label: "Commands", summary: cmds.map((c) => c.name).join(", "), colorClass: "origin-system" };
        }
        case "session_info_update":
          return { icon: "\u{2139}\uFE0F", label: "Session Info", summary: (update as unknown as { title?: string }).title ?? "", colorClass: "origin-system" };
        default: {
          const su = (update as Record<string, unknown>).sessionUpdate as string ?? "";
          if (su === "turn_start") return { icon: "\u25B6", label: "Turn Start", summary: "", colorClass: "origin-system" };
          if (su === "turn_end") return { icon: "\u23F9", label: "Turn End", summary: "", colorClass: "origin-system" };
          const preview = JSON.stringify(update).slice(0, 60);
          return { icon: "\u{1F4E6}", label: su || "Update", summary: preview, colorClass: baseColor };
        }
      }
    }
    default: {
